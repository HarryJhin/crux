...
fn external_paths_from_event(dragging_info: *mut Object) -> Option<ExternalPaths> {
    ...
    if filenames != nil {
        ...
        return Some(ExternalPaths(paths));
    }

    // Fallback: check for image data (PNG or TIFF) on the drag pasteboard.
    // Apps like Preview.app or Photos may provide raw image data without file paths.
    // Save to a temp file so the drop handler receives a usable path.
    if let Some(path) = save_drag_image_to_temp(pasteboard) {
        paths.push(path);
        return Some(ExternalPaths(paths));
    }
...
/// Extract image data from a drag pasteboard and save it to a temp file.
fn save_drag_image_to_temp(pasteboard: id) -> Option<PathBuf> {
    use std::io::Write;
    ...
    let dir = std::env::temp_dir().join("gpui-image-drop");
    std::fs::create_dir_all(&dir).ok()?;

    static COUNTER: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
    let seq = COUNTER.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    let pid = std::process::id();
    let path = dir.join(format!("drop-{pid}-{seq}.{ext}"));
